      ...
      <ul className="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 flex-grow gap-10 p-8">
        {products.map((product) => (
          <Link key={product.id} to={`/${product.id}`}>
            <li
              className="bg-white border border-gray-200 rounded-3xl shadow-md hover:shadow-xl  duration-300 
            ...
            >
              ...
              <div className="p-6 space-y-3">
                ...
                <button
                  className="mt-3 w-full text-sm font-medium text-white bg-gradient-to-r from-slate-800 
                    to-slate-500 py-2 px-4 rounded-full shadow-sm hover:from-slate-600 hover:to-slate-400 transition-all"
                >
                  View Details
                </button>
              </div>
